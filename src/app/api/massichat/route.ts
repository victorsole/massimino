...
export async function GET(request: NextRequest) {
  try {
    ...
    // Determine target user (for trainers viewing athlete sessions)
    let targetUserId = session.user.id;
    if (athleteId && session.user.role === 'TRAINER') {
      // Verify trainer has access to this athlete
      const relationship = await (prisma as any).trainer_clients.findFirst({
    ...
    const sess = await db.ai_chat_sessions.findFirst({ where: { id: sessionId, userId: targetUserId } })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Determine target user (for trainers deleting athlete sessions)
    let targetUserId = session.user.id;
    if (athleteId && session.user.role === 'TRAINER') {
      // Verify trainer has access to this athlete
      const relationship = await (prisma as any).trainer_clients.findFirst({
    ...
    const res = await db.ai_chat_sessions.deleteMany({ where: { id: sessionId, userId: targetUserId } })
